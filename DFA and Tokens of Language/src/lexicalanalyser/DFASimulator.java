...
public class DFASimulator {
	...
	public State isAccepted(Collection<TransitionTable> tt, char[] sourceFileStream) throws Exception {
	    System.out.println("Myfile : " + new String(sourceFileStream));

	    for (TransitionTable table : tt) {
	        State currentState = table.getStartState();
	        System.out.println("current state : " + currentState.toString());

	        for (char inputChar : sourceFileStream) {
	            if (Character.isWhitespace(inputChar)) {
	                // Skip white spaces
	                continue;
	            }

	            State nextState = table.move(currentState, inputChar);
	            if (nextState == null) {
