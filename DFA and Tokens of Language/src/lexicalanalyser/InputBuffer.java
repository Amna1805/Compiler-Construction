...
public class InputBuffer {
	...
	public void addInBuff(char dataFromSource) {
		...
		if (dataFromSource == Main.EOF) {
			System.out.println("Buffer before findToken:");
	        printBuffer();
	...
	private void findToken() {
	    try {
            ...
            int check=0;
            while (fpoint < SIZE_OF_BUFFER) {
            	
                if (check >= MAX_TOKEN_SIZE_ALLOWED || buffer[fpoint] == Main.EOF) {
                    break;
                }
                if (buffer[fpoint] == Main.EOF) {
                    System.out.println("End of file reached. Breaking loop.");
                    break;
                }
                if (Character.isWhitespace(buffer[fpoint])) {
                    if (check > 0) {
                        State currentState = dfaSimulator.isAccepted(transitionController.getTransitionTables(), cache);
                        if (currentState != null) {
                            // Get the token name
                            String tokenName = currentState.getCurrentStateName();
                            // Get the token value
                            String tokenValue = new String(Arrays.copyOfRange(buffer, fpoint - check, fpoint));
                            // Add the token to TokenContainer
                            TokenContainer.getInstanceOf().addToken(tokenName, tokenValue, currentlineNumber);
                            System.out.println("Token Value " + tokenName);
                        }
                        check = 0;
                        Arrays.fill(cache, '\0');
                    }
                } else {
                    cache[check] = buffer[fpoint];
                    check++;
                }

                // Print debug information
                System.out.println("Checking character at line " + currentlineNumber +
                        ": Buffer position: " + fpoint +
                        " Character: '" + buffer[fpoint] + "'");
